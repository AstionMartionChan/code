...
import com.kingnetdc.model.EventTypeEnum.PLAY
import com.kingnetdc.model.WindowDisplayMode.{EVENT_TIME, FLUSH_TIME}
import com.kingnetdc.sink.InfluxDBSink
...
import com.kingnetdc.watermelon.utils.{SparkUtils, StringUtils}
...
object DraftingStatistics extends EventStatistics {
  ...
  def main(args: Array[String]) = {
        ...
        eventRDD.mapPartitions( eventIter => {
          ...
          list.toIterator.map( event => {
            val isNewUser = StatisticsUtils.checkNew(
              userKeyValue, event.fieldValueMap.get(OUID).get.toString, event.time, time.milliseconds,
    ...
    mapWithStateStream.flatMap(_.toList).foreachRDD((rdd, batchTime) => {
      val outputRDD = rdd.repartition(rdd.sparkContext.getConf.getInt(SPARK_OUTPUT_PARTITION, DEFAULT_OUTPUT_PARTITION))
      //输出计算结果
