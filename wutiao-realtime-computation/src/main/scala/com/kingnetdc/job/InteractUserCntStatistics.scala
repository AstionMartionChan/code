...
import com.kingnetdc.utils.StatisticsUtils.sparkConfCheck
...
*/
object InteractUserCntStatistics extends EventStatistics {

    private val METRICS_NAME = "user_cnt"
    ...
    override protected val computationRules: List[ComputationRule] = {
        ComputationRule(
            DID, StringType, s"${ACTION}_${EventTypeEnum.READ.toString}", COUNT_DISTINCT.toString,
            Some(Equal(ACTION, EventTypeEnum.READ.toString, StringType))
        ) ::
        ComputationRule(
            DID, StringType, s"${ACTION}_${EventTypeEnum.PLAY.toString}", COUNT_DISTINCT.toString,
            Some(Equal(ACTION, EventTypeEnum.PLAY.toString, StringType))
        ) ::
            ComputationRule(
                DID, StringType, s"${ACTION}_${EventTypeEnum.LIKE.toString}", COUNT_DISTINCT.toString,
                Some(Equal(ACTION, EventTypeEnum.LIKE.toString, StringType))
            ) ::
            ComputationRule(
                DID, StringType, s"${ACTION}_${EventTypeEnum.COMMENT.toString}", COUNT_DISTINCT.toString,
                Some(Equal(ACTION, EventTypeEnum.COMMENT.toString, StringType))
            ) ::
            ComputationRule(
                DID, StringType, s"${ACTION}_${EventTypeEnum.NOINTEREST.toString}", COUNT_DISTINCT.toString,
                Some(Equal(ACTION, EventTypeEnum.NOINTEREST.toString, StringType))
            ) ::
            ComputationRule(
                DID, StringType, s"${ACTION}_${EventTypeEnum.REPORT.toString}", COUNT_DISTINCT.toString,
                Some(Equal(ACTION, EventTypeEnum.REPORT.toString, StringType))
            ) ::
            ComputationRule(
                DID, StringType, s"${ACTION}_${EventTypeEnum.FAVOUR.toString}", COUNT_DISTINCT.toString,
    ...
    def parse(line: String, config: Map[String, String]) = {
        try {
            ...
            if (
                ...
                Some(
                    (logReceiveTime, did, event, appVersion, appChannel, os, itemtype, ptype.toInt, itemid, status)
                )
            } else {
                None
            }
    ...
    private def buildDimensionValues(device_type: String, itemType: String, os: String, sortId: String) = {
        List(
