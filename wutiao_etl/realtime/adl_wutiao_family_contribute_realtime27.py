...
#*作者:gant
#*时间:2018-06-21
#*备注:五条家庭贡献力统计表（统计最近30天家庭贡献力）
# 每天凌晨1点开始统计过去29天的家庭贡献力加上00点到01点，其余时间统计上一个小时的家庭贡献力
#***************************************************************#

import kingnet
import time

#跨零点的时候为了获取前一天的23点分区，所以ds>='{ds}'
sql_hour = '''
...
from(
    ...
    full join(
        ...
        from {sourcetab}
        where ds = '{last_hour_ds}' and hour='{last_hour}' and event in ('joinfamily','leavefamily') 
        group by ouid
    )t2
    on t1.ouid=t2.ouid
)t3
left join(
    ...
    from {sourcetab}
    where ds = '{last_hour_ds}' and hour='{last_hour}' and event = 'contributioninc' and familyid is not null 
    group by ouid
)t4
on t3.ouid=t4.ouid;
'''
...
from(
    ...
    full join(select ouid,sum(contributionpoint) as contributionpoint from {sourcetab2} where ds='{ds}' and event='contributioninc' and familyid is not null group by ouid)tt2
...
hourfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_family_contribute_realtime/ds='{last_hour_ds}'/hour='{last_hour}' '''
dayfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_family_contribute_realtime/ds='{ds}'/hour=29day '''

def main():
    kdc = kingnet.kdc()
    ds = kdc.workDate
    today = kdc.dateAdd(1,ds)
