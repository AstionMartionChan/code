...
# **********************程序说明*********************************#
# *模块: adl_wutiao_total_new_uid_etl > ADL
# *功能: 临时计算实时账号累计天表
# *作者: sunyu
# *时间: 2018-07-20
...
CREATE TABLE `realtime_total_new_uid_etl` (
...
UID_SQL = '''
select dds, '%(statdate)s' as statdate, last_appver, channel, os, sum(act_uv) as act_uv, sum(new_uv) as new_uv, sum(act_uv)-sum(new_uv) as lost_uv
from (
    select dds
    , nvl(last_appver,'allappver') as last_appver
    , nvl(channel, 'allchannel') as channel
    , nvl(last_os, 'allos') as os
    , count(distinct ouid) as act_uv
    , 0 as new_uv
    from (
        select ds as dds
            , ouid
        ...
        where ds = '%(today)s'
        and nvl(ouid,'-1') != '-1' 
        and event in ('comment','favour','like','share','read','click','nointerest','report','attention','leaveread','play','search','login','active','enterfront','openclient','register') 
        and date_format(from_unixtime(cast(round(_sst/1000.0,0) as bigint)) AT TIME ZONE 'Asia/Shanghai','%%Y-%%m-%%d %%H:%%i:%%s') between '%(today)s 00:00:00' and '%(end_statdate)s'
        and eventtype in ('super', 'high')
        group by ouid, ds
    ) t
    GROUP BY GROUPING SETS(
            (dds, last_appver, channel, last_os),
            (dds)
    )
    union all 
    select new_ds as dds
        , nvl(last_appver,'allappver') as last_appver
        , nvl(channel, 'allchannel') as channel
        , nvl(last_os, 'allos') as os
        , 0 as act_uv
        , count(distinct ouid) as new_uv
    from (
        ...
        (
            ...
            and nvl(ouid,'-1') != '-1' 
            and date_format(from_unixtime(cast(round(_sst/1000.0,0) as bigint)) AT TIME ZONE 'Asia/Shanghai','%%Y-%%m-%%d %%H:%%i:%%s') >'%(some_day)s 00:00:00'
            and date_format(from_unixtime(cast(round(_sst/1000.0,0) as bigint)) AT TIME ZONE 'Asia/Shanghai','%%Y-%%m-%%d %%H:%%i:%%s') <'%(end_statdate)s'
    ...
    where new_ds = '%(today)s'
    GROUP BY GROUPING SETS(
            (new_ds, last_appver, channel, last_os),
            (new_ds)
    )
) t
group by dds, last_appver, channel, os
'''
...
def main(time_str, auto_just):
    ...
    args = {
        ...
        'day1': kdc.workDate,
        'today': kdc.dateAdd(1),
        'uid_mysql_tb': 'wutiao.realtime_total_new_uid_etl',
    }
    ...
    try:
        ...
        if send_just:
            kingnetdc.send_wechat('五条离线方式计算实时累计活跃账号脚本失败', 'sunyu@kingnet.com,zhangxj@kingnet.com,zhangfan@kingnet.com')
            kingnetdc.send_phone('五条离线方式计算实时累计活跃账号脚本失败: %s ' % time_str, ['13918035392', '13761810576'])
            raise Exception(msg)
