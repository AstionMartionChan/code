...
full join(
     ...
     maxbytimenotnull(if(event='invite' and inviterid<>0 and type in (2,4) and step='0',inviterid,null),-1*cast(substr(\`_sst\`,1,10) as bigint)) as inviterid,
     min(if(event='invite' and inviterid<>0 and type in (2,4) and step='0',cast(substr(\`_sst\`,1,10) as bigint),null)) as inviter_time
     from {sourcetab2}
     where ds='{ds}' and event in('contributioninc','verifypointinc','register','active','login','invite','openclient','enterfront')
     group by ouid
)t2
on t1.ouid=t2.ouid;
'''
