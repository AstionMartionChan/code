...
CREATE TABLE `realtime_new_user_etl` (
...
select  dds, statdate
    ...
    from (
        select dds, statdate, a.ouid
            ...
            , new_tag
        from (
            select '%(today)s' as dds, '%(statdate)s' as statdate, ouid
...
def main(time_str, auto_just):
    ...
    args = {
        ...
        'day1': kdc.workDate,
        'today': kdc.dateAdd(1),
        'uid_mysql_tb': 'wutiao.realtime_new_user_etl',
    }
    ...
    try:
        ...
        if send_just:
            kingnetdc.send_wechat('五条离线方式计算实时活跃用户脚本失败', 'sunyu@kingnet.com,zhangxj@kingnet.com,zhangfan@kingnet.com')
            kingnetdc.send_phone('五条离线方式计算实时活跃用户脚本失败: %s ' % time_str, ['13918035392', '13761810576'])
            raise Exception(msg)
