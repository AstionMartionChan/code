...
from {sourcetab} 
where ds = '{ds}' and eventtype in ('super') and ouid is not null and event = 'media' 
group by ouid)t1
full outer join
...
hourfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao.db/adl_wutiao_money_realtime/ds='{last_hour_ds}'/hour='{last_hour}' '''
dayfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao.db/adl_wutiao_money_realtime/ds='{ds}'/hour=99 '''
touchz_check = '''hadoop fs -touchz /user/hive/warehouse/check_point/'{last_hour_ds}'-'{last_hour_str}'_adl_wutiao_money_realtime'''
moneygain_yesterday = '''hive -e "select uid, money_gain from {targettab} where ds='{ds}' and hour='99' and money_gain>0"|tr "\t" "|"|grep -v 'WARN\|hadoop' > /data/wwwroot/wutiao-hadooplog.kyhub.cn/getmoney_{ds}.txt'''
moneygain_info = '''echo `md5sum /data/wwwroot/wutiao-hadooplog.kyhub.cn/getmoney_%(ds)s.txt|awk -F " " '{print $1}'`"|"`wc -l /data/wwwroot/wutiao-hadooplog.kyhub.cn/getmoney_%(ds)s.txt|awk -F " " '{print $1}'` > /data/wwwroot/wutiao-hadooplog.kyhub.cn/getmoney_%(ds)s_info.txt'''
...
def main():
    ...
    sqlmedia_hour = sql_media.format(ds=last_hour_ds,targettab='wutiao.idl_wutiao_user_status', sourcetab='wutiao.odl_event_qkl')
    sqlmedia_day = sql_media.format(ds=ds,targettab='wutiao.idl_wutiao_user_status', sourcetab='wutiao.odl_event_qkl')
    moneygain_yesterday_cmd = moneygain_yesterday.format(ds=ds,targettab='wutiao.adl_wutiao_money_realtime')
