...
left join(
    ...
    from {sourcetab2}
    where ds='{last_hour_ds}' and hour<='{last_hour}' and event in('verifypointinc','like') and ouid is not null
    group by ouid
)t2
on t1.uid=t2.ouid;
'''
...
left join(
    ...
    sum(if(event='like' and type=3,1,0)) as vote_cnt
    from {sourcetab2}
    where ds='{ds}' and event in('verifypointinc','like') and ouid is not null
    group by ouid
)t2
on t1.uid=t2.ouid;
'''
...
hourfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_discover_moneygain_rank_realtime/ds='{last_hour_ds}'/hour='{last_hour}' '''
dayfile_check = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_discover_moneygain_rank_realtime/ds='{ds}'/hour=99 '''
hourfile_dependency = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_money_realtime/ds='{last_hour_ds}'/hour='{last_hour}' '''
dayfile_dependency = '''hadoop fs -test -e /user/hive/warehouse/wutiao_db/adl_wutiao_money_realtime/ds='{ds}'/hour=99 '''
...
group by tt1.uid,first_time order by money_gain desc limit 100)t1;"|tr "\t" "|" | grep -v 'WARN\|hadoop' > /opt/etl/wutiao_file/getmoney_rank_{ds}.txt'''
moneygain_7day_info = '''echo `md5sum /opt/etl/wutiao_file/getmoney_rank_%(ds)s.txt|awk -F " " '{print $1}'`"|"`wc -l /opt/etl/wutiao_file/getmoney_rank_%(ds)s.txt|awk -F " " '{print $1}'` > /opt/etl/wutiao_file/getmoney_rank_%(ds)s_info.txt'''
...
on t1.uid=t2.uid)t3 where rn=1;"|tr "\t" "|" | grep -v 'WARN\|hadoop' > /opt/etl/wutiao_file/getmoney_last_10_{ds}.txt'''
meonygain_last10_info = '''echo `md5sum /opt/etl/wutiao_file/getmoney_last_10_%(ds)s.txt|awk -F " " '{print $1}'`"|"`wc -l /opt/etl/wutiao_file/getmoney_last_10_%(ds)s.txt|awk -F " " '{print $1}'` > /opt/etl/wutiao_file/getmoney_last_10_%(ds)s_info.txt'''
...
def main():
    ...
    hourfile_dependency_flag = kdc.doCommand(hourfile_d, True)
    dayfile_dependency_flag = kdc.doCommand(dayfile_d, True)
