...
database="wutiao"
table="bdl_wutiao_event"

hive <<EOF
use $database;
create table if not exists $table (
ouid                      string     comment    '用户id'
,ts                       bigint     comment    '业务行为时间'
,serversystime            bigint     comment    '服务器时间'
...
,osver                    string     comment    '客户端操作系统版本'
,model                    string     comment    '设备型号'
,mfr                      string     comment    '设备'
,res                      string     comment    '分辨率'
,nettype                  string     comment    '设备网络类型'
,carrier                  string     comment    '设备运营商'
,channel                  string     comment    '安装渠道'
...
,accemeter                string     comment    '加速度'
,magfield                 string     comment    '磁力'
,orient                   string     comment    '方向'
,gyros                    string     comment    '陀螺仪'
,light                    string     comment    '光线感应'
,press                    string     comment    '压力'
,tempera                  string     comment    '温度'
,prox                     string     comment    '距离感应'
,grav                     string     comment    '重力'
,lineacce                 string     comment    '线性加速度'
,rota                     string     comment    '旋转矢量'
,gps                      string     comment    'GPS位置'
,sort                     bigint     comment    '对于这个文章id该用户是第几个举报的'
,usertype                 int        comment    '用户的类型，1：普通用户，2：发现者用户'
,coin                     double     comment    '金币数'
,rate                     double     comment    '汇率'
,rmb                      double     comment    '提现的rmb'
,sdk                      string     comment    'sdk'
,sdkver                   string     comment    'sdk版本'
...
,market_channel_type      string     comment    '市场渠道类型'
)comment '区块链事件源表'
partitioned by (eventtype string comment '日志事件等级', ds string comment '日期分区字段, yyyy-MM-dd')
