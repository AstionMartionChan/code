...
from project_constant import DB_PARAMS_WUTIAO_USER, DB_PARAMS_WUTIAO_USER_HOSTS, WUTIAO_USER_TAB_SYNC_INFO
...
def sync_data(mysqlhosts, db_params, outfile, mysql_query, load_data):
    for mysqlhost in mysqlhosts:
        ...
        try:
            cmd = """mysql -h%s -u%s -p%s -P%i -N -C -q -e"%s" > %s""" % (
                db_params['host'], db_params['user'], db_params['password'], db_params['port'], mysql_query, outfile)
            kingnetdc.exec_cmd(cmd)
...
def main():
    ...
    args['db_params'] = DB_PARAMS_WUTIAO_USER
    args['mysqlhosts'] = DB_PARAMS_WUTIAO_USER_HOSTS
    args['mysqltabs'] = WUTIAO_USER_TAB_SYNC_INFO
