...
def main():
    ...
    sync_sql = '''
        select fds, appver, channel, os
            , nvl(lost_activeuser_cnt,0)
            , nvl(silent_user_cnt,0)
            , nvl(day1_lost_user_cnt,0)
            , nvl(day3_lost_user_cnt,0)
            , nvl(day7_lost_user_cnt,0)
            , nvl(day30_lost_user_cnt,0)
            , nvl(lost_newuser_cnt,0)
            , nvl(lost_olduser_cnt,0)
        from %(targettab)s
        where ds between '%(day30)s' and '%(run_date)s'
    ''' % args
