...
his_income,
null as uid,
day_discover_vote_income,
his_discover_vote_income
from(
    select itemid,
    ...
    from {sourcetab1}
    where ds='{ds}' and eventtype in ('super','high') and event in('comment','like','share','read','validread','report','nointerest','play')
    union all
    select ts,event,status,type,split(itemid,'@')[0] as itemid,if(split(itemid,'@')[2]='article',1,2) as itemtype 
    from (select ts,event,status,type,itemlist from {sourcetab1} where ds='{ds}' and eventtype='high' and event='favour')t1 
    ...
    from (select ts,event,status,type,itemlist from {sourcetab1} where ds='{ds}' and eventtype='low' and event='showinpage')t1 
    lateral view explode (split(itemlist,','))item as itemid
    )tt1
...
null as uid,
nvl(tt1.day_discover_vote_income,0) as day_discover_vote_income,
nvl(tt1.day_discover_vote_income,0)+nvl(tt2.his_discover_vote_income,0) as his_discover_vote_income
from(
    select nvl(t1.itemid,t2.itemid) as itemid,
    ...
    nvl(t1.discover_vote_income,0) as day_discover_vote_income
    from(
        select itemid,
        ...
        sum(if(actiontype=15,value,0)) as discover_vote_income,
        sum(value) as day_income
        from {sourcetab2}
        where ds = '{ds}' and actiontype in(10,11,12,13,14,15) and itemid is not null
        group by itemid
    )t1
    full outer join(
        select itemid,
        ...
        from {sourcetab1} 
        where ds = '{ds}' and eventtype='high' and event in ('contributioninc','verifypointinc') and itemid is not null
        group by itemid
    )t2
    on cast(t1.itemid as bigint)=cast(t2.itemid as bigint)
)tt1
...
left join (select item_id,uid from {sourcetab3} where ds='{ds}')t2
