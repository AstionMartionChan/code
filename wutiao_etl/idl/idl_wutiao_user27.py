...
,maxbytimenotnull(if(event in('login','active','enterfront','openclient','register'), did, null), ts) as last_did_ouid        
...
,min(if(event in('login','active','openclient','register') or (event='enterfront' and staytime>30000),ts, null)) as first_login_time      
,max(if(event in('login','active','openclient','register') or (event='enterfront' and staytime>30000),ts, null)) as last_login_time
,sum(if(event in('login','active','openclient','register') or (event='enterfront' and staytime>30000),1, 0)) as last_login_cnt          
...
,sum(if(event = 'enterbackground',1, 0)) as enterbackground_cnt   
,sum(if(event = 'enterbackground',staytime, 0)) as usetime               
...
,maxbytimenotnull(if(event='invite' and inviterid<>0 and type in (2,4) and step='0',inviterid,null),-1*ts) as inviterid
,min(if(event='invite' and inviterid<>0 and type in (2,4) and step='0',ts,null)) as inviter_time  
...
,min(if(event='withdraw' and ts>=1531843200 and status=2,ts,null)) as first_withdraw_time
,max(if(event='withdraw' and ts>=1531843200 and status=2,ts,null)) as last_withdraw_time
,sum(if(event='withdraw' and ts>=1531843200 and status=2,rmb,0)) as last_day_withdraw_rmb
,sum(if(event='withdraw' and ts>=1531843200 and status=2,coin,0)) as last_day_withdraw_coin
...
,max(if(event='getwb' and ts>=1531827000 and status=2,ts,null)) as last_getwb_time
,sum(if(event='getwb' and ts>=1531827000 and status=2,coin,0)) as last_day_getwb_wb
from {sourcetab} 
where ds = '{ds}' and nvl(ouid,'-1')<>'-1' 
...
,maxbytimenotnull(if(event in('login','active','enterfront','openclient','register'), ouid, null), ts) as last_did_ouid        
,maxbytimenotnull(if(event in('login','active','enterfront','openclient','register'), appver, null), -1*ts) as first_appver          
,maxbytimenotnull(if(event in('login','active','enterfront','openclient','register'), appver, null), ts) as last_appver           
,min(if(event='register' and step=0,ts,null)) as registe_time          
,min(if(event in('login','active','openclient','register') or (event='enterfront' and staytime>30000),ts, null)) as first_login_time      
...
,min(if(event = 'discoverer' and status=1,ts,null)) as first_discover_time
,maxbytimenotnull(if(event = 'discoverer' and status=1,1,status),ts) as is_discover
,min(if(event='withdraw' and status=2,ts,null)) as first_withdraw_time
,max(if(event='withdraw' and status=2,ts,null)) as last_withdraw_time
,sum(if(event='withdraw' and status=2,rmb,0)) as last_day_withdraw_rmb
,sum(if(event='withdraw' and status=2,coin,0)) as last_day_withdraw_coin
,min(if(event='report' and status=1,ts,null)) as  first_report_time
,max(if(event='report' and status=1,ts,null)) as last_report_time
,sum(if(event='report' and status=1,1,0)) as last_report_cnt
,min(if(event='nointerest',ts,null)) as first_nointerest_time
,max(if(event='nointerest',ts,null)) as last_nointerest_time
,sum(if(event='nointerest',1,0)) as last_nointerest_cnt    
,max(if(event in('login','active','enterfront','openclient','register') and ouid is not null,ts, null)) as last_did_ouid_time
,sum(if(event = 'enterfront',1, 0)) as enterfront_cnt   
,sum(if(event = 'enterfront',staytime, 0)) as gaptime
from {sourcetab} 
...
left join(
    ...
    from {sourcetab}
    where ds = '{ds}' and event in ('joinfamily','leavefamily')
    group by ouid
)t2
on t1.ouid=t2.ouid;
...
def main():
    ...
    if res != 0:
        raise Exception("utiao.idl_wutiao_user insert error")
